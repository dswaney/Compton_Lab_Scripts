...
#          idempotent disabling of HP BIOS network/Internet firmware updates,
...
$script:AvAutoPowerOn = [pscustomobject]@{ Status = 'NotRun'; Time = '00:00'; Days = 'Sunday,Monday,Tuesday,Wednesday,Thursday,Friday'; Vendor = $null; Message = $null }
$script:HpNetworkBiosUpdate = [pscustomobject]@{
    Status      = 'NotRun'
    SettingName = $null
    Before      = $null
    After       = $null
...
$script:DellWakeOnLan = [pscustomobject]@{
    Status               = 'NotRun'
    BiosOption           = $null
    BiosBefore           = $null
    BiosAfter            = $null
...
function Get-HpiaRecommendationObjects {
    ...
    # Use an in-place mutable collection here. Add-StructuredRecommendation is
    # a nested function, so `$results += ...` creates/rebinds a child-scope
    # variable and silently loses every parsed item when that function returns.
    ...
    function Find-JsonRecommendationCollections {
        ...
        foreach ($property in $Value.PSObject.Properties) {
            if ($property.Name -eq 'Recommendations') {
                $parserState.StructuredCandidateCount += @($property.Value).Count
    ...
    Write-Log ("HPIA structured report collections contained {0} recommendation candidate(s)." -f $parserState.StructuredCandidateCount) 'INFO'
    Add-YamlAction ("HPIA structured report collections contained {0} recommendation candidate(s)." -f $parserState.StructuredCandidateCount)

    if ($parserState.StructuredCandidateCount -gt 0 -and $results.Count -eq 0) {
        throw ("HPIA report contained {0} recommendation candidate(s), but the structured parser retained none. Installation is blocked to prevent a false-success result." -f $parserState.StructuredCandidateCount)
    }
...
# -----------------------------
# AV computer firmware power-on schedule
# -----------------------------
function Get-HpBcuSetting {
    param(
        [AllowEmptyString()]
    ...
    for ($i = 0; $i -lt $Lines.Count; $i++) {
        $settingLine = [string]$Lines[$i]
        if ($settingLine.Trim() -ine $Name) { continue }
        ...
        for ($j = $i + 1; $j -lt $Lines.Count; $j++) {
            ...
            $value = $raw.Trim()
            if ($null -eq $firstValue) { $firstValue = $value.TrimStart('*') }
            if ($value.StartsWith('*')) {
                $selected = $value.TrimStart('*')
                if ($selected -match '^(?i:Enabled?)$') { return 'Enable' }
                if ($selected -match '^(?i:Disabled?)$') { return 'Disable' }
                return $selected
            }
        }

        if ($firstValue -match '^(?i:Enabled?)$') { return 'Enable' }
        if ($firstValue -match '^(?i:Disabled?)$') { return 'Disable' }
        return $firstValue
    }
    return $null
}

function Find-HpNetworkBiosUpdateSettingName {
    param(
        [Parameter(Mandatory)]
        [AllowEmptyString()]
        [AllowEmptyCollection()]
    ...
    foreach ($preferredName in $preferredNames) {
        if ($null -ne (Get-HpBcuSetting -Lines $Lines -Name $preferredName)) {
            return $preferredName
        }
    }

    # BIOS labels can vary by model and BIOS generation. Only accept a single,
    # unambiguous setting that mentions both BIOS updates and network/Internet.
    $matches = @(
        $Lines |
        Where-Object {
            $_ -and $_ -notmatch '^\s' -and
            $_.Trim() -match '(?i)(BIOS.*Update|Update.*BIOS)' -and
            $_.Trim() -match '(?i)Network|Internet'
        } |
...
function Get-HpBcuSettingOptions {
    param(
        [AllowEmptyString()]
        [AllowEmptyCollection()]
        [string[]]$Lines,
    ...
    for ($i = 0; $i -lt $Lines.Count; $i++) {
        ...
        for ($j = $i + 1; $j -lt $Lines.Count; $j++) {
            ...
            if ([string]::IsNullOrWhiteSpace($raw)) { continue }
            if ($raw -notmatch '^\s') { break }
            $options.Add($raw.Trim().TrimStart('*')) | Out-Null
        }
...
function Invoke-HpNetworkBiosUpdatePolicy {
    Write-Section 'HP BIOS Network Update Policy'

    $tool = Join-Path $HpBiosConfigShare 'BIOSConfigUtility64.exe'
    $password = Join-Path $HpBiosConfigShare 'password.bin'

    try {
        if (-not (Test-Path -LiteralPath $tool -PathType Leaf)) {
            throw "HP BIOS Configuration Utility missing: $tool"
        }

        # BCU expects its companion signature DLL beside the executable. The
        # BIOS password file remains on the restricted share and is never copied.
        $local = Join-Path $WorkingRoot 'HpNetworkBiosPolicy'
        Ensure-Folder -Path $local
        foreach ($file in @(Get-ChildItem -LiteralPath $HpBiosConfigShare -File -ErrorAction Stop |
            Where-Object { $_.Name -match '^(BIOSConfigUtility64\.exe|BCUsignature64\.dll)$' })) {
            Copy-Item -LiteralPath $file.FullName -Destination $local -Force -ErrorAction Stop
        }

        $bcu = Join-Path $local 'BIOSConfigUtility64.exe'
        if (-not (Test-Path -LiteralPath $bcu -PathType Leaf)) {
            throw 'HP BIOS Configuration Utility local staging failed.'
        }

        $beforePath = Join-Path $local 'bios-network-update-before.txt'
        $afterPath = Join-Path $local 'bios-network-update-after.txt'
        $settingsPath = Join-Path $local 'disable-bios-network-update.txt'
        Remove-Item -LiteralPath $beforePath, $afterPath, $settingsPath -Force -ErrorAction SilentlyContinue

        $output = @(& $bcu "/Get:$beforePath" 2>&1)
        $exitCode = $LASTEXITCODE
        if ($exitCode -ne 0 -or -not (Test-Path -LiteralPath $beforePath -PathType Leaf)) {
            throw "HP BIOS read failed (exit $exitCode): $($output -join ' ')"
        }

        $beforeLines = @(Get-Content -LiteralPath $beforePath -ErrorAction Stop)
        $populatedLineCount = @(
            $beforeLines |
            Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) }
        ).Count
        if ($populatedLineCount -eq 0) {
            throw 'HP BIOS Configuration Utility created an empty BIOS export.'
        }

        $settingName = Find-HpNetworkBiosUpdateSettingName -Lines $beforeLines
        if ([string]::IsNullOrWhiteSpace($settingName)) {
            $script:HpNetworkBiosUpdate.Status = 'NotSupported'
            $script:HpNetworkBiosUpdate.Message = 'This HP BIOS does not expose an unambiguous network BIOS update setting through BCU.'
            Write-Log $script:HpNetworkBiosUpdate.Message 'WARN'
            Add-YamlAction 'HP BIOS network update policy was not changed because the setting is not exposed or is ambiguous on this model.'
            return
        }

        $script:HpNetworkBiosUpdate.SettingName = $settingName
        $script:HpNetworkBiosUpdate.Before = Get-HpBcuSetting -Lines $beforeLines -Name $settingName

        if ($script:HpNetworkBiosUpdate.Before -eq 'Disable') {
            $script:HpNetworkBiosUpdate.Status = 'AlreadyDisabled'
            $script:HpNetworkBiosUpdate.After = 'Disable'
            $script:HpNetworkBiosUpdate.Message = 'Network/Internet BIOS updates were already disabled; no BIOS write was performed.'
            Write-Log $script:HpNetworkBiosUpdate.Message 'OK'
            Add-YamlAction "HP BIOS setting '$settingName' was already disabled; no change was needed."
            return
        }

        if (-not (Test-Path -LiteralPath $password -PathType Leaf)) {
            throw "HP BIOS password file missing: $password"
        }

        # Preserve the model's actual list of accepted values. Older HP BIOS
        # versions use several Automatic BIOS Update choices rather than a
        # simple Enable/Disable pair. REPSET requires literal tab indentation.
        $settingOptions = @(Get-HpBcuSettingOptions -Lines $beforeLines -Name $settingName)
        $disableOption = @(
            $settingOptions |
            Where-Object { $_ -match '^(?i:Disabled?)$' }
        ) | Select-Object -First 1
        if ([string]::IsNullOrWhiteSpace([string]$disableOption)) {
            throw "HP BIOS setting '$settingName' does not offer a Disable value; no change was attempted."
        }
        ...
        $configLines.Add($settingName) | Out-Null
        foreach ($option in $settingOptions) {
            if ($option -ieq $disableOption) {
                $configLines.Add("`t*$option") | Out-Null
            }
            else {
                $configLines.Add("`t$option") | Out-Null
            }
        }
        Set-Content -LiteralPath $settingsPath -Value $configLines -Encoding ASCII -ErrorAction Stop

        $output = @(& $bcu "/Set:$settingsPath" "/cpwdfile:$password" '/WarningAsErr' 2>&1)
        $exitCode = $LASTEXITCODE
        if ($exitCode -ne 0) {
            throw "HP BIOS network update setting failed (exit $exitCode): $($output -join ' ')"
        }

        $output = @(& $bcu "/Get:$afterPath" 2>&1)
        $exitCode = $LASTEXITCODE
        if ($exitCode -ne 0 -or -not (Test-Path -LiteralPath $afterPath -PathType Leaf)) {
            throw "HP BIOS write completed but readback failed (exit $exitCode): $($output -join ' ')"
        }

        $afterLines = @(Get-Content -LiteralPath $afterPath -ErrorAction Stop)
        $script:HpNetworkBiosUpdate.After = Get-HpBcuSetting -Lines $afterLines -Name $settingName
        if ($script:HpNetworkBiosUpdate.After -ne 'Disable') {
            throw "HP BIOS readback mismatch for '$settingName'. Expected Disable; found '$($script:HpNetworkBiosUpdate.After)'."
        }

        $script:HpNetworkBiosUpdate.Status = 'Configured'
        $script:HpNetworkBiosUpdate.Message = 'Network/Internet BIOS updates were disabled and verified.'
        Write-Log $script:HpNetworkBiosUpdate.Message 'OK'
        Add-YamlAction "HP BIOS setting '$settingName' changed from '$($script:HpNetworkBiosUpdate.Before)' to 'Disable' and was verified."
    }
    catch {
        $script:HpNetworkBiosUpdate.Status = 'Failed'
        $script:HpNetworkBiosUpdate.Message = $_.Exception.Message
        Add-RunFailure "HP BIOS network update policy: $($_.Exception.Message)"
    }
}

function Set-HpAvAutoPowerOn {
    $tool = Join-Path $HpBiosConfigShare 'BIOSConfigUtility64.exe'
    $password = Join-Path $HpBiosConfigShare 'password.bin'
    if (-not (Test-Path -LiteralPath $tool -PathType Leaf)) { throw "HP BIOS Configuration Utility missing: $tool" }
    if (-not (Test-Path -LiteralPath $password -PathType Leaf)) { throw "HP BIOS password file missing: $password" }

    # BCU expects its companion DLLs beside the executable. Copy only the
    # utility files; keep password.bin on the restricted share.
    $local = Join-Path $WorkingRoot 'AvBiosConfig'
    Ensure-Folder -Path $local
    foreach ($file in @(Get-ChildItem -LiteralPath $HpBiosConfigShare -File -ErrorAction Stop |
        Where-Object { $_.Name -match '^(BIOSConfigUtility64\.exe|BCUsignature64\.dll)$' })) {
        Copy-Item -LiteralPath $file.FullName -Destination $local -Force -ErrorAction Stop
    ...
    $bcu = Join-Path $local 'BIOSConfigUtility64.exe'
    if (-not (Test-Path -LiteralPath $bcu)) { throw 'HP BIOS utility copy failed.' }
    $before = Join-Path $local 'bios-before.txt'
    $after = Join-Path $local 'bios-after.txt'
    $settings = Join-Path $local 'av-power-on.txt'
    Remove-Item -LiteralPath $before, $after -Force -ErrorAction SilentlyContinue
    $output = @(& $bcu "/Get:$before" 2>&1)
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $before)) {
...
function Set-DellAvAutoPowerOn {
    if (-not (Test-Path -LiteralPath $DellCommandConfigureShare -PathType Container)) {
        throw "Dell Command Configure share not accessible: $DellCommandConfigureShare"
    }
    $sourcePath = Join-Path $DellCommandConfigureShare 'X86_64\cctk.exe'
    if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) {
        throw "64-bit Dell Command Configure executable not found: $sourcePath"
    }
    $source = Get-Item -LiteralPath $sourcePath -ErrorAction Stop

    # cctk requires adjacent binaries; stage its extracted directory locally.
    $local = Join-Path $WorkingRoot 'AvDellConfigure'
    Ensure-Folder -Path $local
    Copy-Item -Path (Join-Path $source.DirectoryName '*') -Destination $local -Recurse -Force -ErrorAction Stop
    $cctk = Join-Path $local 'cctk.exe'
    if (-not (Test-Path -LiteralPath $cctk -PathType Leaf)) { throw 'Dell Configure local staging failed.' }

    $before = @{}
    foreach ($option in @('AutoOn', 'AutoOnHr', 'AutoOnMn')) {
        $query = @(& $cctk "--$option" 2>&1)
        if ($LASTEXITCODE -ne 0) { throw "Dell BIOS $option query failed (exit $LASTEXITCODE): $($query -join ' ')" }
        $before[$option] = $query -join "`n"
    }
    if ($before.AutoOn -match '(?im)^\s*AutoOn\s*=\s*SelectDays\s*:\s*([^\r\n]+)') {
        $days = @($Matches[1] -split '\s*,\s*' | ForEach-Object { $_.Trim().ToLowerInvariant() } | Sort-Object -Unique)
        if (($days -join ',') -eq 'fri,mon,sun,thu,tue,wed' -and
            $before.AutoOnHr -match '(?im)^\s*AutoOnHr\s*=\s*0\s*$' -and
            $before.AutoOnMn -match '(?im)^\s*AutoOnMn\s*=\s*0\s*$') { return 'AlreadyCompliant' }
    }

    # Dell documents SelectDays:Day,Day and one shared hour/minute.
    $desired = '--AutoOn=SelectDays:Sun,Mon,Tue,Wed,Thu,Fri'
    $output = @(& $cctk $desired '--AutoOnHr=0' '--AutoOnMn=0' 2>&1)
    if ($LASTEXITCODE -ne 0) { throw "Dell BIOS AutoOn configuration failed (exit $LASTEXITCODE): $($output -join ' ')" }
    $after = @{}
    foreach ($option in @('AutoOn', 'AutoOnHr', 'AutoOnMn')) {
        $readback = @(& $cctk "--$option" 2>&1)
        if ($LASTEXITCODE -ne 0) { throw "Dell BIOS $option readback failed (exit $LASTEXITCODE): $($readback -join ' ')" }
        $after[$option] = $readback -join "`n"
    }
    if ($after.AutoOn -notmatch '(?im)^\s*AutoOn\s*=\s*SelectDays\s*:\s*([^\r\n]+)') { throw 'Dell BIOS readback did not report SelectDays.' }
    $days = @($Matches[1] -split '\s*,\s*' | ForEach-Object { $_.Trim().ToLowerInvariant() } | Sort-Object -Unique)
    if (($days -join ',') -ne 'fri,mon,sun,thu,tue,wed' -or
        $after.AutoOnHr -notmatch '(?im)^\s*AutoOnHr\s*=\s*0\s*$' -or
        $after.AutoOnMn -notmatch '(?im)^\s*AutoOnMn\s*=\s*0\s*$') {
        throw 'Dell BIOS readback did not match the Sunday/weekday midnight schedule.'
    }
    return 'Configured'
}

function Get-DellCommandConfigureLocalPath {
    param([string]$Purpose = 'DellConfigure')
    ...
    $source = Get-Item -LiteralPath $sourcePath -ErrorAction Stop
    $local = Join-Path $WorkingRoot $Purpose
    Ensure-Folder -Path $local
    Copy-Item -Path (Join-Path $source.DirectoryName '*') -Destination $local -Recurse -Force -ErrorAction Stop
    $cctk = Join-Path $local 'cctk.exe'
    if (-not (Test-Path -LiteralPath $cctk -PathType Leaf)) { throw 'Dell Command Configure local staging failed.' }
    return $cctk
}

function Get-DellCctkOptionValue {
    param(
        [Parameter(Mandatory)][string[]]$Output,
        [Parameter(Mandatory)][string]$OptionName
    )
    $text = $Output -join "`n"
    if ($text -match ("(?im)^\s*{0}\s*=\s*([^\r\n]+)" -f [regex]::Escape($OptionName))) {
        return $Matches[1].Trim()
    }
    return $null
}

function Invoke-DellWakeOnLanPolicy {
    Write-Section 'Dell Wired Wake-on-LAN Policy'
    try {
        $cctk = Get-DellCommandConfigureLocalPath -Purpose 'DellWakeOnLan'
        $selectedOption = $null
        $beforeValue = $null
        foreach ($candidate in @('WakeOnLan', 'WakeOnLan2')) {
            $query = @(& $cctk "--$candidate" 2>&1)
            $queryExitCode = $LASTEXITCODE
            if ($queryExitCode -ne 0) {
                Write-Log ("Dell BIOS option {0} is not available on this model (exit {1})." -f $candidate, $queryExitCode) 'INFO'
                continue
            }
            $value = Get-DellCctkOptionValue -Output $query -OptionName $candidate
        ...
        if (-not $selectedOption) {
            throw 'Neither WakeOnLan nor WakeOnLan2 is exposed by Dell Command Configure on this model.'
        }

        $script:DellWakeOnLan.BiosOption = $selectedOption
        $script:DellWakeOnLan.BiosBefore = $beforeValue
        $wiredEnabledValues = @('LanOnly','LanWlan','LanWithPxeBoot','OnBoard','AddInCard','Enable','Enabled')
        if ($wiredEnabledValues -contains $beforeValue) {
            $script:DellWakeOnLan.BiosAfter = $beforeValue
            $script:DellWakeOnLan.BiosCompliant = $true
            Write-Log ("Dell BIOS {0} already permits wired Wake-on-LAN: {1}." -f $selectedOption, $beforeValue) 'OK'
        }
        else {
            $setOutput = @(& $cctk ("--{0}=LanOnly" -f $selectedOption) 2>&1)
            $setExitCode = $LASTEXITCODE
            if ($setExitCode -ne 0) {
                throw "Dell BIOS $selectedOption could not be set to LanOnly (exit $setExitCode): $($setOutput -join ' ')"
            }
            $verifyOutput = @(& $cctk "--$selectedOption" 2>&1)
            $verifyExitCode = $LASTEXITCODE
            if ($verifyExitCode -ne 0) {
                throw "Dell BIOS $selectedOption readback failed (exit $verifyExitCode): $($verifyOutput -join ' ')"
            }
            $afterValue = Get-DellCctkOptionValue -Output $verifyOutput -OptionName $selectedOption
            if ($wiredEnabledValues -notcontains $afterValue) {
                throw "Dell BIOS Wake-on-LAN readback is not wired-enabled. Expected LanOnly; found '$afterValue'."
            }
            $script:DellWakeOnLan.BiosAfter = $afterValue
            $script:DellWakeOnLan.BiosCompliant = $true
            Write-Log ("Dell BIOS {0} changed from {1} to {2} and verified." -f $selectedOption, $beforeValue, $afterValue) 'OK'
        }
        ...
        $physicalAdapters = @(
            ...
            Where-Object {
                ...
                (($_.Name + ' ' + $_.InterfaceDescription) -notmatch '(?i)wireless|wi-?fi|wlan|bluetooth|wwan|mobile')
            }
        )
        $script:DellWakeOnLan.EthernetAdapterCount = $physicalAdapters.Count
        if ($physicalAdapters.Count -eq 0) { throw 'No physical Ethernet adapter was detected for Wake-on-LAN verification.' }

        foreach ($adapter in $physicalAdapters) {
            $beforePower = Get-NetAdapterPowerManagement -Name $adapter.Name -ErrorAction Stop
            $beforeMagic = [string]$beforePower.WakeOnMagicPacket
            $status = 'AlreadyEnabled'
            $message = $null
            if ($beforeMagic -notmatch '^(?i:Enabled)$') {
                try {
            ...
            $afterPower = Get-NetAdapterPowerManagement -Name $adapter.Name -ErrorAction SilentlyContinue
            $afterMagic = if ($afterPower) { [string]$afterPower.WakeOnMagicPacket } else { $null }
            $verified = ($afterMagic -match '^(?i:Enabled)$')
            if (-not $verified -and $status -ne 'Failed') {
                $status = 'Failed'
                $message = 'WakeOnMagicPacket did not verify as Enabled.'
            }
            $adapterResults.Add([pscustomobject]@{
                ...
                WakeOnMagicPacketBefore = $beforeMagic
                WakeOnMagicPacketAfter = $afterMagic
        ...
        $adapterWasChanged = @($adapterResults | Where-Object { $_.Status -eq 'Configured' }).Count -gt 0
        $script:DellWakeOnLan.Status = if (($script:DellWakeOnLan.BiosBefore -eq $script:DellWakeOnLan.BiosAfter) -and -not $adapterWasChanged) { 'AlreadyCompliant' } else { 'Configured' }
        $script:DellWakeOnLan.Message = 'Dell BIOS and physical Ethernet adapter Wake-on-LAN settings were verified.'
        Write-Log $script:DellWakeOnLan.Message 'OK'
        Add-YamlAction $script:DellWakeOnLan.Message
    }
    catch {
        $script:DellWakeOnLan.Status = 'Failed'
        $script:DellWakeOnLan.Message = $_.Exception.Message
        Write-Log ("Dell Wake-on-LAN policy failed: {0}" -f $_.Exception.Message) 'WARN'
        Add-RunFailure ("Dell Wake-on-LAN policy: {0}" -f $_.Exception.Message)
    }
}

function Invoke-AvAutoPowerOn {
    if ($script:ComputerName -notmatch '(?i)(^|[-_])AV($|[-_])' -or $SkipAvAutoPowerOn) {
        $script:AvAutoPowerOn.Status = 'Skipped'
        return
    }
    Write-Section 'AV BIOS Auto Power-On'
    $script:AvAutoPowerOn.Vendor = $script:DetectedVendor
    try {
        switch ($script:DetectedVendor) {
            'HP'   { $script:AvAutoPowerOn.Status = Set-HpAvAutoPowerOn }
            'Dell' { $script:AvAutoPowerOn.Status = Set-DellAvAutoPowerOn }
            default { throw "Auto power-on not implemented for vendor $($script:DetectedVendor)." }
        }
        Write-Log "AV BIOS schedule $($script:AvAutoPowerOn.Status): Sunday and Monday-Friday at 00:00; Saturday off." 'OK'
        Add-YamlAction "AV BIOS auto power-on $($script:AvAutoPowerOn.Status): Sunday and Monday-Friday 00:00; Saturday off."
    }
    catch {
        $script:AvAutoPowerOn.Status = 'Failed'
        $script:AvAutoPowerOn.Message = $_.Exception.Message
        Add-RunFailure "AV BIOS auto power-on: $($_.Exception.Message)"
    }
}
...
try {
    ...
    if ($script:DetectedVendor -eq 'HP') {
        Invoke-HpNetworkBiosUpdatePolicy
    }

    Invoke-AvAutoPowerOn
