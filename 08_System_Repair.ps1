...
Import-Module -Name $CopilotModulePath -Force -DisableNameChecking -ErrorAction Stop
...
# Canonical Copilot implementation. This later definition supersedes the
# legacy in-file routine while preserving the existing opt-in switch.
function Invoke-CopilotDisableAndRemoval {
    ...
    Add-RepairAttempt 'Microsoft Copilot disable and removal'
    Add-DetailedResult -Step 'CopilotDisableAndRemoval' `
        -Status $(if ($copilotResult.Failures -gt 0) { 'Warning' } else { 'Info' }) `
        -Message "Shared Microsoft Copilot removal completed with status $($copilotResult.Status)." `
        -Data @{
            ModuleVersion = $copilotResult.ModuleVersion
            Status = $copilotResult.Status
            InstalledPackagesRemoved = $copilotResult.InstalledPackagesRemoved
            ProvisionedPackagesRemoved = $copilotResult.ProvisionedPackagesRemoved
            ProcessesStopped = $copilotResult.ProcessesStopped
            TasksDisabled = $copilotResult.TasksDisabled
            UserHivesUpdated = $copilotResult.UserHivesUpdated
            OfflineUserHivesUpdated = $copilotResult.OfflineUserHivesUpdated
            ShortcutsRemoved = $copilotResult.ShortcutsRemoved
            Failures = $copilotResult.Failures
        }
}
